 ...
 * a real number of distinct params.permit! mass-assignment findings flagged -- never on install, never on
...
const KEY_SEEN_FINDINGS = 'railsMassAssignmentCompanion.review.seenFindings';
const KEY_ANSWERED = 'railsMassAssignmentCompanion.review.answered';
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Rails Mass Assignment Companion: this has flagged ${HITS_BEFORE_PROMPT} real issues in your code -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
